...
% Load SC-SNr inactivation recordings
load('latSC_val.mat');
...
%% Do a hierarchical bootstrap for the IQR and plot for SNr-SC

num_shuffles = 1000;
mean_firing_rate_win_rand = nan(numel(sp_times_filt), 2, num_shuffles);
num_neurons = numel(sp_struct_filt);
parfor x = 1:num_shuffles

    disp(x)

    rand_ind = randi(numel(sp_times_filt), 1, numel(sp_times_filt));
    
    sp_times_filt_rand = sp_times_filt(rand_ind);
    laser_trial_rand = laser_trial(rand_ind);
