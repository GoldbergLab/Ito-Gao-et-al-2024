function [plot_data_summary,plot_data_indiv,shuffle_test_laser_tailOne,shuffle_test_laser_tailTwo,shuffle_test_position_tailOne,shuffle_test_position_tailTwo] = plot_bilateral_opto(data_set,lick_num,recentering,ylabel_input,ylim_val,min_max_angle,plot_individual,contact_dur_opto,lick_index_contact_num,L2contact_only,mm_pix,bin_size, fig_name)

load(data_set)
prot_angle = cell(numel(t_stats_combined), 2, 3, numel(lick_num));
med_prot_angle_all = cell(numel(t_stats_combined), 2, 3, numel(lick_num));
% prot_angle_all_laser = cell(numel(t_stats_combined), 3, numel(lick_num));
% prot_angle_all_position = cell(numel(t_stats_combined), 2, numel(lick_num));
...
shuffle_test_laser_tailOne = nan(numShuffles,size(shuf_med_prot_angle_all,3));
shuffle_test_laser_tailTwo = nan(numShuffles,size(shuf_med_prot_angle_all,3));
...
for i = 1:numShuffles
    shuffle_test_laser_tailOne(i,:) = shuf_med_prot_angle_all(i,1,:) - shuf_med_prot_angle_all(i,2,:) <= 0;
    shuffle_test_laser_tailTwo(i,:) = shuf_med_prot_angle_all(i,1,:) - shuf_med_prot_angle_all(i,2,:) >= 0;
...
shuffle_test_laser_tailOne = mean(shuffle_test_laser_tailOne,1);
shuffle_test_laser_tailTwo = mean(shuffle_test_laser_tailTwo,1);
