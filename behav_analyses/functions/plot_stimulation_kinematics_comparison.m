function [plot_data_summary,plot_data_indiv,shuffled_test_results] = plot_stimulation_kinematics_comparison(data_set_cued,data_set_uncued,ylabel_input,lick_num,ylim_val, fig_name)
    ...
    for j=1:gridSize(2)
        ...
        for k=1:numel(data_cued)
            for l = 1:2
                ...
                if l == 1
                    data_indiv = data_cued(k).data;
                elseif l == 2
                    data_indiv = data_uncued(k).data;
                end
...
stats_cued=cell(1,numel(data_cued));
stats_ctrl=cell(1,numel(data_cued));
stats_UNcued=cell(1,numel(data_cued));
